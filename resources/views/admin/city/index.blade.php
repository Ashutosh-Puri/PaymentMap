        ...
        <div class="col m-0 p-0">
            ...
            <div class="card">
                <div class="card-header bg-primary fw-bold text-white">
                    View Cities
                    <a class="btn btn-success fw-bold float-end" href="{{ route('admin.city.create') }}">Create</a>
                </div>
                <div class="card-body">
                        ...
                        @if (isset($cities))
                            @forelse ($cities as $i)
                                <tr>
                                    ...
                                    <td>
                                        <a class="btn btn-primary btn-sm fw-bold" href="{{ route('admin.city.edit',$i->id) }}">Edit</a>

                                        <a class="btn btn-danger btn-sm fw-bold" href="javascript:void(0)" onclick="$(this).parent().find('form').submit()">Delete</a>
                                        <form action="{{ route('admin.city.destroy',$i->id) }}" method="post">
                                            @csrf
                                            @method('DELETE')
                                        </form>
                                    </td>
